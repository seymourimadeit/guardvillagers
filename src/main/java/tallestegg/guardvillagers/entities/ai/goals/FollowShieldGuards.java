    ...
    public boolean canUse() {
        ...
        if (!list.isEmpty()) {
            for (Guard guard : list) {
                if (!(guard.getMainHandItem().getItem() instanceof ProjectileWeaponItem) && !guard.isInvisible() && guard.getOffhandItem().canPerformAction(net.minecraftforge.common.ToolActions.SHIELD_BLOCK) && guard.isBlocking()
